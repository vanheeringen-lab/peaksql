...
class BedRegionDataSet(_BedDataSet):
    ...
    def __init__(self, database: str, where: str = "", seq_length: int = 200,
                 label_func: str = 'any', **kwargs: int):
        _BedDataSet.__init__(self, database, where, seq_length, label_func=label_func,
                             **kwargs)
    ...
    def array_from_query(self, query, cur_chrom_id, chromstart, chromend):
        ...
        for chromosome_id, condition_id, start, end in query:
            if cur_chrom_id == chromosome_id:
                ...
                max_idx = end - chromstart if end - chromstart <= positions.shape[1] else positions.shape[1]
